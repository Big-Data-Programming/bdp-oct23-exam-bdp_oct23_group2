...
def send_exam_link(account_id, email_id):
    ...
    html_message = render_to_string('exam_link_email.html', context)
    plain_message = strip_tags(html_message)
    send_mail(subject, plain_message, 'from@example.com', [email_id], html_message=html_message)

def send_interview_invitation(email_id):
    subject = 'Congratulations on passing the coding challenge.'
    meeting_time = datetime.now() + timedelta(days=15)
    context = {
        'meeting_time': meeting_time.strftime('%Y-%m-%d'),
        'meeting_link': "https://www.microsoft.com/en/microsoft-teams/log-in"
    }
    html_message = render_to_string('interview_invitation_email.html', context)
    plain_message = strip_tags(html_message)
    send_mail(subject, plain_message, 'from@example.com', [email_id], html_message=html_message)

def send_test_response(email_id):
    subject = 'Thank you for your time.'
    html_message = render_to_string('test_response_email.html')
    plain_message = strip_tags(html_message)
    send_mail(subject, plain_message, 'from@example.com', [email_id], html_message=html_message)
